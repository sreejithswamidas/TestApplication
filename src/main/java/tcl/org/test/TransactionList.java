...
public class TransactionList {
   ...
   public StatisticsModel setStatisticsModel(StatisticsModel statisticsModel){
       List<TransactionModel> transactionstemp=transactions.stream().filter(t->checkTimeStamp(t.getTimestamp())).collect(Collectors.toList());
       count=0;
       total=0;
       average=0;
       max=0;
       min=100;
        if(!transactionstemp.isEmpty()) {
            count = (int) transactionstemp.stream().count();
            total = (float) transactionstemp.stream().mapToDouble(TransactionModel::getAmount).sum();
            average = (float) transactionstemp.stream().mapToDouble(TransactionModel::getAmount).average().getAsDouble();
            min = (float) transactionstemp.stream().mapToDouble(TransactionModel::getAmount).min().getAsDouble();
            max = (float) transactionstemp.stream().mapToDouble(TransactionModel::getAmount).max().getAsDouble();
